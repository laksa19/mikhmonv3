...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    $onlogin = ':put (",'.$expmode.',' . $price . ',' . $validity . ',,,' . $getlock . ',"); {:global date [ /system clock get date ];:global year [ :pick $date 7 11 ];:global month [ :pick $date 0 3 ];:global comment [ /ip hotspot user get $user comment]; :local ucode [:pic $comment 0 3]; :if ($ucode = "vc-" or $ucode = "up-" or $comment = "") do={ /sys sch add name="$user" disable=no start-date=$date interval="' . $validity . '"; :delay 2s; :local exp [ /sys sch get [ /sys sch find where name="$user" ] next-run]; :local getxp [len $exp]; :if ($getxp = 15) do={ :local d [:pic $exp 0 6]; :local t [:pic $exp 7 16]; :local s ("/"); :local exp ("$d$s$year $t"); /ip hotspot user set comment=$exp [find where name="$user"];}; :if ($getxp = 8) do={ /ip hotspot user set comment="$date $exp" [find where name="$user"];}; :if ($getxp > 15) do={ /ip hotspot user set comment=$exp [find where name="$user"];}; /sys sch remove [find where name="$user"]';
