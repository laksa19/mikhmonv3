...
<div class="card-header">
	<h3><i class=" fa fa-align-justify"></i> User Log <?php echo $idhr.$idbl;?></h3>
</div>
<div class="card-body">
	<div>
		...
		<div class="input-group mr-b-10">  
			...
			<select style="padding:5px;" class="group-item group-item-md" title="Month" id="M">
        			<?php 
        			$idbls =array(1=>"jan","feb","mar","apr","may", "jun","jul","aug","sep","okt", "nov","des");
        			...
        			for ($x = 1; $x <= 12; $x++) {echo "<option value='".$idbls[$x]."''>".$idblf[$x]."</option>";}
        			?> 
    		</select>
			</div>
			...
			<script type="text/javascript">
				...
				function filterR(){
					...
					if(D !== ""){
						window.location='./app.php?hotspot=userlog&idhr='+M+'/'+D+'/'+Y+'&session=<?php echo $session;?>';
					}else if(D === ""){
						window.location='./app.php?hotspot=userlog&idbl='+M+Y+'&session=<?php echo $session;?>';
					}
				}
			</script>
		</div>
